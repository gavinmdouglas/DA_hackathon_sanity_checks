...
for(dataset in datasets_to_test) {
  ...
  dataset_filt_abun_wilcoxon_sig <- wilcoxon_2group_sig_features(dataset_filt_abun_filename, dataset_group_tab_filename, convert_CLR = TRUE)
  
  dataset_filt_wilcoxon_orig <- read.table(paste("/home/jacob/projects/HACKATHON_ANCOM_FIX_21_03_13/Hackathon/Studies/",
                                                 dataset,
                                                 "/Fix_Results_0.1/Wilcoxon_CLR_out/Wil_CLR_results.tsv",
                                                 sep = ""),
  ...
  dataset_unfilt_abun_wilcoxon_sig <- wilcoxon_2group_sig_features(dataset_unfilt_abun_filename, dataset_group_tab_filename, convert_CLR = TRUE)
  
  dataset_unfilt_wilcoxon_orig <- read.table(paste("/home/jacob/projects/HACKATHON_ANCOM_FIX_21_03_13/Hackathon/Studies/",
                                                   dataset,
                                                   "/No_filt_Results/Wilcoxon_CLR_out/Wil_CLR_results.tsv",
                                                   sep = ""),
                                             header = TRUE, sep = "\t", check.names = FALSE, row.names = 1)
